...
# Modified by: Dylan Mumm, 9-10-20, CPSC 4420
...
#############################################################################
    ...
    # builds complete closed_set, dictionary for nodes and their parents
    def DFS_find(parNode, curNode, closed_set, open_set, obstacles, grid_size, start, goal):
        ...
        d = ((curNode[0]-1, curNode[1]), (curNode[0], curNode[1]-1), # up, left
            (curNode[0]+1, curNode[1]), (curNode[0], curNode[1]+1))  # down, right

        neighbors = (i for i in d if i not in obstacles and i not in closed_set and i not in open_set and
                     i[1] < grid_size[1] and i[0] < grid_size[0] and i[1] >= 0 and i[0] >= 0)
        for i in neighbors:
            open_set.add(i)
            parentOf[i] = curNode
        if open_set:
            nextNode = open_set.pop()
            DFS_find(curNode, nextNode, closed_set, open_set, obstacles, grid_size, start, goal)

    # starting from goal, works back through dictionary to start to build path
    # returns move list
    def DFS_path(curNode, moves):

        p = parentOf[curNode]
        lastAction = (curNode[0]-p[0], curNode[1]-p[1])
        moves.append(lastAction)

        if p != start:
            return DFS_path(p, moves)
    ...
    parentOf = {}
    DFS_find(start, start, closed_set, open_set, obstacles, grid_size, start, goal)
    movement = DFS_path(goal, movement)
...
#############################################################################
    ...
    # builds complete closed_set, dictionary for nodes and their parents
    def UCS_find(parNode, curNode, closed_set, open_set, obstacles, grid_size, start, goal, costFn):

        closed_set.add(curNode)
        if curNode == goal:
            return True

        d = ((curNode[0]-1, curNode[1]),
            (curNode[0], curNode[1]-1), # up, left
            (curNode[0]+1, curNode[1]),
            (curNode[0], curNode[1]+1))  # down, right

        neighbors = (i for i in d if i not in obstacles and i not in closed_set and i not in open_set and
                     i[1] < grid_size[1] and i[0] < grid_size[0] and i[1] >= 0 and i[0] >= 0)

        for i in neighbors:
            open_set.put(i, costFn(i))
            parentOf[i] = curNode

        if open_set:
            nextNode = open_set.pop()[0]
            UCS_find(curNode, nextNode, closed_set, open_set, obstacles, grid_size, start, goal, costFn)

    # starting from goal, works back through dictionary to start to build path
    # returns move list
    def UCS_path(curNode, moves):
        p = parentOf[curNode]
        lastAction = (curNode[0]-p[0], curNode[1]-p[1])
        moves.append(lastAction)
        if p != start:
            return UCS_path(p, moves)
        else:
            moves.reverse()
            return moves

    parentOf = {}
    UCS_find(start, start, closed_set, open_set, obstacles, grid_size, start, goal, costFn)
    movement = UCS_path(goal, movement)
...
#############################################################################
    ...
    # builds complete closed_set, dictionary for nodes and their parents
    def ASS_find(parNode, curNode, closed_set, open_set, obstacles, grid_size, start, goal, costFn):
        ...
        for i in neighbors:
            open_set.put(i, costFn(i) + heuristic(i[0], i[1]))
            parentOf[i] = curNode

        if open_set:
            nextNode = open_set.pop()[0]
            ASS_find(curNode, nextNode, closed_set, open_set, obstacles, grid_size, start, goal, costFn)

    # starting from goal, works back through dictionary to start to build path
    # returns move list
    def ASS_path(curNode, moves):
    ...
    print(obstacles)
    parentOf = {}
    ASS_find(start, start, closed_set, open_set, obstacles, grid_size, start, goal, costFn)
    movement = ASS_path(goal, movement)

...
#
#	# <i-th> MISSING IPC CALL <with optional comment>
#
...
#
#   For values of n higher than 4, use:
#
#       mpiexec --use-hwthread-cpus -n 8 python mpics.py
#
...
if rank != 0:
	...
	for i in range( 1, 3 ):
		...
		# first MISSING IPC CALL  # to server with update request
		comm.Send(request, dest=0)


		# second MISSING IPC CALL  # from server for reply
	...
	# third MISSING IPC CALL  # to server with completion notice
	comm.Send(request, dest=0)

	# fourth MISSING IPC CALL  # from server for reply
	comm.Recv(reply, source=0)
...
# server code
	...
	while num_clients > 0:
		...
		# fifth MISSING IPC CALL  # from any client
		...
		# sixth MISSING IPC CALL  # reply back to that client

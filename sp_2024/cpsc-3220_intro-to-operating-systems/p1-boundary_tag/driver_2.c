/* CPSC 3220-002 - Spring 2024 - program 1 - driver 2
 *
 * tests of free list going empty and being restored
 *
 * release_mem() tested for cases 1 and 2 
 *   1 - both above and below are allocated
 *   2 - above is free and below is allocated
 *
...
int main(){
  ...
  printf("memory allocation test 2, pointer size is %lu bytes\n",
    sizeof(void *));

  init_region();
  prt_free_list();

  printf("alloc 0x640 - tests getting all of the initial free block\n");
  ptr[0] = alloc_mem(0x640); if(ptr[0]==NULL) printf("ptr[0] gets NULL\n");
  prt_free_list();
  printf("release 0x640 - put it back\n");
  rc=release_mem(ptr[0]); if(rc) printf("*** release_mem() fails\n");
  prt_free_list();

  printf("alloc 6 blocks of 0xe0 each (0x100 bytes after split)\n");
  ...
  printf("try to alloc 0x50 more\n");
  ptr[7] = alloc_mem(0x50);
  if(ptr[7]==NULL) printf("alloc_mem() returns NULL - as expected\n");

  printf("try to alloc 0x20 - should result in empty free list\n");
  ptr[8] = alloc_mem(0x20);
  if(ptr[8]==NULL) printf("*** alloc_mem() returns NULL\n");

  prt_free_list();

  printf("release ptr[8] - tests adding to empty free list\n");
  rc=release_mem(ptr[8]); if(rc) printf("*** release_mem(ptr[8]) fails\n");
  prt_free_list();

  printf("release ptr[6] - tests adding to free list with free above\n");
  rc=release_mem(ptr[6]); if(rc) printf("*** release_mem(ptr[6]) fails\n");
  prt_free_list();

  printf("release other blocks - should have restored free list\n");
  rc=release_mem(ptr[5]); if(rc) printf("*** release_mem(ptr[5]) fails\n");
  rc=release_mem(ptr[4]); if(rc) printf("*** release_mem(ptr[4]) fails\n");
  rc=release_mem(ptr[3]); if(rc) printf("*** release_mem(ptr[3]) fails\n");
  rc=release_mem(ptr[2]); if(rc) printf("*** release_mem(ptr[2]) fails\n");
  rc=release_mem(ptr[1]); if(rc) printf("*** release_mem(ptr[1]) fails\n");
  prt_free_list();

  printf("try everything a second time to make sure free list not corrupted\n");

  ptr[10] = alloc_mem(0x640); if(ptr[10]==NULL) printf("ptr[10] gets NULL\n");
  prt_free_list();
  rc=release_mem(ptr[10]); if(rc) printf("*** release_mem() fails\n");
  prt_free_list();

  ptr[11] = alloc_mem(0xe0); if(ptr[11]==NULL) printf("ptr[11] gets NULL\n");
  ptr[12] = alloc_mem(0xe0); if(ptr[12]==NULL) printf("ptr[12] gets NULL\n");
  ptr[13] = alloc_mem(0xe0); if(ptr[13]==NULL) printf("ptr[13] gets NULL\n");
  ptr[14] = alloc_mem(0xe0); if(ptr[14]==NULL) printf("ptr[14] gets NULL\n");
  ptr[15] = alloc_mem(0xe0); if(ptr[15]==NULL) printf("ptr[15] gets NULL\n");
  ptr[16] = alloc_mem(0xe0); if(ptr[16]==NULL) printf("ptr[16] gets NULL\n");
  ptr[17] = alloc_mem(0x20); if(ptr[17]==NULL) printf("ptr[17] gets NULL\n");
  prt_free_list();

  rc=release_mem(ptr[17]); if(rc) printf("*** release_mem(ptr[18]) fails\n");
  rc=release_mem(ptr[16]); if(rc) printf("*** release_mem(ptr[16]) fails\n");
  rc=release_mem(ptr[15]); if(rc) printf("*** release_mem(ptr[15]) fails\n");
  rc=release_mem(ptr[14]); if(rc) printf("*** release_mem(ptr[14]) fails\n");
  rc=release_mem(ptr[13]); if(rc) printf("*** release_mem(ptr[13]) fails\n");
  rc=release_mem(ptr[12]); if(rc) printf("*** release_mem(ptr[12]) fails\n");
  rc=release_mem(ptr[11]); if(rc) printf("*** release_mem(ptr[11]) fails\n");
  prt_free_list();

...
put:
	...
	ldr r1, .L12		@loading the address of count
	ldr r2, [r1] 		@loading the value of count

	cmp r5, #8			@comparing count to CBUFSIZE (8)
	
	bne not_full		@branching if cbuf is not full
	
	ldr r0, .L12 + 4	@printing cbuf is full
	bl printf			
...
not_full:
	...
	ldr r3, .L12 + 8	@loading address of in	
	ldr r4, [r3]		@loading value of in
	ldr r5, .L12 + 12	@loading address of cbuf

	str r0, [r5, r4]  	@store val in cbuf (r5) offset by in(r4) cbuf[in] = val
	
	add r4, r4, #1		@in = in + 1
	and r4, r4, #7		@anding in with cmask (7)
	str r4, [r3]		@savig in to global variable
	
	add r2, r2, #1		@incrementing count
	str r2, [r1]		@storing count globally
...
get:
	...
	ldr r0, .L17		@loading address of count 
	ldr r1, [r0]		@loading value of count

	cmp r1, #0			@if (count == 0)
	bne not_empty1

	ldr r0, .L17+4			@printing "empty"	
	bl printf
	mov r0, #0 				@returning null
	b done1

not_empty1:
	ldr r2, .L17 + 8		@loading address of out
	ldr r3, [r2]			@loading value of out
	ldr r4, .L17 + 12		@loading address of cbuf
	ldr r0, [r4, r3] 		@loading value of cbuf at out

	add r3, r3, #1			@out = out + 1
	and r3, r3, #7			@out = out & CBUFMASK
	str r3, [r2]			@storing out globally

	ldr r2, .L17			@reloading the address of count
	sub r1, r1, #1			@count--
	str r1, [r2]			@storing count globablly

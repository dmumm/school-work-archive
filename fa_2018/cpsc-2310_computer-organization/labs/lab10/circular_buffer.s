...
put:
	...
	ldr r1, .L12       @ loading count address
	ldr r2, [r1]       @ loading dereferenced address
	cmp r5, #8				 @ if r5 < CBUFSIZE
	bne vacant         @ ---> vacant

	ldr r0, .L12 + 4   @ printing that cbuf is full
	bl printf
	b done

vacant:
	ldr r3, .L12 + 8    @ loading in address
	ldr r4, [r3]        @ dereferencing address
	ldr r5, .L12 + 12   @ loading cbuf address
	str r0, [r5, r4]    @ storing cbuf offset by in

	add r4, r4, #1    @ in++
	and r4, r4, #7    @ in = in & CBUFMASK
	str r4, [r3]      @ storing in

	add r2, r2, #1    @ count++
	str r2, [r1]			@ storing count
...
get:
	...
	ldr r0, .L17  @ loading count address
	ldr r1, [r0]  @ loading dereferenced address
	cmp r1, #0    @ if count is 0
	bne next      @ branch to next
...
next:
	ldr r2, .L17 + 8   @ loading out address
	ldr r3, [r2]       @ loading dereferenced address
	ldr r4, .L17 + 12  @ loading cbuf address
	ldr r0, [r4, r3]   @ loading cbuf offset by out

	add r3, r3, #1   @ out++
	and r3, r3, #7   @ out = out & CBUFMASK
	str r3, [r2]     @ storing out

	ldr r2, .L17		 @ loading count address
	sub r1, r1, #1   @ count--
	str r1, [r2]     @ storing count
